...
<p style="font-size:17px;color:#bbb;line-height:1.8;">Premium oversized streetwear hoodie mit softem Stoff und modernem Fit. Entwickelt fuer einen cleanen und stylischen Everyday Look.</p>
...
<button id="popupCartBtn" style="background:#f5b6c8;color:#111;padding:16px 28px;border:none;border-radius:999px;font-weight:bold;font-size:16px;cursor:pointer;">In den Warenkorb</button>
<a href="warenkorb.html" style="background:#1f1f24;color:white;padding:16px 28px;border-radius:999px;font-weight:bold;">Zum Warenkorb</a>
</div>
<div style="margin-top:30px;color:#aaa;line-height:1.9;">✓ Oversized Fit<br>✓ Premium Cotton<br>✓ Soft Inside Fabric<br>✓ Designed by elina</div>
</div>
</div>
</div>
`;
...
document.querySelectorAll(".colorSwitch").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    if(color === "blue") popupMainImage.style.filter = "hue-rotate(180deg)";
    else if(color === "red") popupMainImage.style.filter = "hue-rotate(320deg)";
    else if(color === "pink") popupMainImage.style.filter = "hue-rotate(260deg)";
    else popupMainImage.style.filter = "none";
  });
});
...
function updateCart() {
  ...
  cart.forEach((item, index) => {
    ...
    div.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;width:100%;gap:20px;background:#18181d;padding:14px;border-radius:18px;margin-bottom:14px;">
      ...
      <button onclick="removeItem(${index})" style="background:#ef4444;color:white;border:none;padding:12px 16px;border-radius:14px;cursor:pointer;font-weight:bold;">
        Löschen
      </button>
